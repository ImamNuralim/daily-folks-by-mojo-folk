...
const NewsSlider = () => {
  ...
  if (error) return <div className="text-center py-10 text-blue-500">Gagal memuat: {error}</div>;
  ...
  // Nama Kategori (Ambil kategori pertama)
  const categoryName = activePost?._embedded?.['wp:term']?.[0]?.[0]?.name || 'Uncategorized';
  const categoryId = activePost?._embedded?.['wp:term']?.[0]?.[0]?.id;

  return (
    <div className="relative w-auto overflow-hidden rounded-lg shadow-xl mb-8 group mt-20">
      {/* 1. Konten Slider Aktif */}
      <div className="  relative w-400 h-96 sm:h-[450px] md:h-[550px]">
        ...
        <div 
          ...
          style={{ 
            ...
            // Cek apakah post memiliki featured image
        ...
        <div className="absolute bottom-0 left-0 p-6 md:p-12 w-full lg:w-3/4">
          {categoryId && (
             <Link 
              ...
              className="inline-block px-3 py-1 mb-3 text-xs font-bold uppercase text-white bg-blue-600 hover:bg-blue-700 rounded-full transition duration-150"
            >
              {categoryName}
            </Link>
          )}
          
          <Link to={`/post/${activePost.slug}`}>
            <h2 
              className="text-white text-3xl md:text-5xl font-extrabold leading-tight hover:text-blue-300 transition duration-300"
      ...
      <button 
        onClick={prevSlide}
        className="absolute left-4 top-1/2 transform -translate-y-1/2 p-3 bg-black/50 text-white rounded-full hover:bg-blue-600 transition duration-300 opacity-0 group-hover:opacity-100"
      >
      ...
      <button 
        onClick={nextSlide}
        className="absolute right-4 top-1/2 transform -translate-y-1/2 p-3 bg-black/50 text-white rounded-full hover:bg-blue-600 transition duration-300 opacity-0 group-hover:opacity-100"
      >
        ...
        {posts.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all duration-300 ${
              index === currentSlide ? 'bg-blue-600 w-6' : 'bg-white/50 hover:bg-white'
            }`}
